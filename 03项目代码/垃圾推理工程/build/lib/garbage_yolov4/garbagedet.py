from garbage_yolov4.models import Darknet    # YOLOv4网络结构的实现
from garbage_yolov4.utils.datasets import *  # 引入所有数据集处理函数
from garbage_yolov4.utils.utils import *     # 引入yolo的计算函数：比如iou计算
...
class GarbageDetector:
    ...
    def __init__(self):
        super(GarbageDetector ,self).__init__()
        # 条件1：文件准备
        current_dir = os.path.dirname(__file__)
        self.cfg_file = os.path.join(current_dir, "data/yolov4-tiny.cfg")
        self.mod_file = os.path.join(current_dir, "data/garbages.pt")
        self.names_file = os.path.join(current_dir, "data/garbages.names")
